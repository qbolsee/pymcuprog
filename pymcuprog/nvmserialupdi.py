...
from .pymcuprog_errors import PymcuprogSessionError
...
class NvmAccessProviderSerial(NvmAccessProvider):
    ...
    def read_device_id(self):
        ...
        if not self.device_info.get(DeviceInfoKeysAvr.DEVICE_ID) == device_id_read:
            self.logger.warning("Device ID mismatch: read out '{0:X}'; expected '{1:X}'.".format(device_id_read,
    ...
    def write(self, memory_info, offset, data, blocksize=0):
        ...
        while data_aligned:
            ...
            if memtype_string == MemoryNames.LOCKBITS:
                ...
                bulk = 1
                if n_chunk == 1:
                    # if only one chunk, it is NOT a bulk write.
                    bulk = 0
